...
async function init() {
  ...
  const { word: wordRes } = await res.json();  //making an object 
  const word = wordRes.toUpperCase(); //making every letter to upper case
  const wordParts = word.split(""); //spliting the each character of the word
  isLoading = false;
  ...
  function addLetter(letter) {
    if (currentGuess.length < ANSWER_LENGTH) {
      currentGuess += letter; // we are adding the letter 
    } else {
      current = currentGuess.substring(0, currentGuess.length - 1) + letter;  // handiling the last character 
    }
    letters[currentRow * ANSWER_LENGTH + currentGuess.length - 1].innerText = letter; // here we are able to write or show text on the screen 
  }
  ...
  async function commit() {
    ...
    // first we pass just finds correct letters so we can mark those as correct first
    ...
    // we use the map to make sure we mark the correct amount of close letters (here close letter means if there is only 1 O but we give 2 O so it consider that O which is in tere correct position or the first one )
    for (let i = 0; i < ANSWER_LENGTH; i++) {
      if (guessParts[i] === wordParts[i]) {
        // do nothing
    ...
    if (allRight) {
      // win
      document.querySelector(".won").classList.add("finish");

      document.querySelector(".brand").classList.add("finish");
      done = true;
    } else if (currentRow === ROUNDS) {
      // lose
      document.querySelector(".loss").classList.add("loss");
